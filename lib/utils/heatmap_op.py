...
def select_topk(heat_map, K=100):
    ...
    # topk_inds_all = topk_inds_all % (height * width) #
    topk_ys = torch.floor_divide(topk_inds_all, width).float()
    ...
    topk_clses = torch.floor_divide(topk_inds, K).float()
...
def select_point_of_interest(batch, index, feature_maps):
    ...
    Args:
        batch: batch size
        index: in point format or index format # (bs,30,2)
